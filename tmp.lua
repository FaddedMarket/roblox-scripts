...
local GetStockYPrices        = RemoteEvents:FindFirstChild("GetStockYPrices")
...
local ChangeDisplaySneakerEvent = RemoteEvents:FindFirstChild("ChangeDisplaySneaker")
local RemoveDisplaySneakerEvent = RemoteEvents:FindFirstChild("RemoveDisplaySneaker")
...
local Settings = {
    ...
    SnipeMode              = false,
    OneDollarMode          = false,
    DryRun                 = savedSettings.DryRun                 ~= nil and savedSettings.DryRun                 or false,
...
local function saveSetting(key, value)
    if key == "StockYMargin" or key == "MaxPrice" or key == "Delay"
       ...
       or key == "BuyConfirmPollWait" or key == "OverpayLimit" then
...
task.defer(function()
    ...
    for _, def in ipairs(tabDefs) do
        ...
        if frame then tabContainerRegistry[def.tab] = frame
        else warn("[ShoeSniper] Could not find ScrollingFrame for tab: " .. def.name) end
    end
end)
...
local function freezeStaticUI(tab)
    ...
    if not frame then
        task.spawn(function()
            ...
            if not frame then
                warn("[ShoeSniper] freezeStaticUI: could not find frame for tab")
                return
            end
            local snap = {}
...
local function isUnsellableByModule(shoeName)
    ...
    local data = SneakerModule.sneakers[shoeName]
...
local function buildUnsellableCache()
    if SneakerModuleLoaded and SneakerModule then
        local c = 0
        for _, data in pairs(SneakerModule.sneakers) do
            if table.find(data, "Unsellable") then c += 1 end
        end
...
local function sendWebhookRaw(payload)
    ...
    pcall(function()
        ...
        if fn then fn({ Url=Settings.WebhookURL, Method="POST",
...
local function sendWebhook(shoeName, price, confirmed, webhookTitle, sellerName)
    ...
    local fields = {
        ...
        { name="Mode",          value=Settings.DryRun and "DRY RUN" or "LIVE",       inline=true  },
...
local function refreshStockY()
    if not GetStockYPrices then log("STOCKY","Remote not found") return end
    local loading = true
    task.spawn(function()
        while loading do Rayfield:Notify({ Title="StockY", Content="Loading...", Duration=6 }) task.wait(5) end
    end)
...
-- ════════════════════════════════════════
--  STOCK-Y CHECK + OVERPAY LIMITER
-- ════════════════════════════════════════
local function stockYOk(shoeName, price)
    if not Settings.UseStockYCheck then return true end
    local sp = StockYCache[shoeName]
    if not sp or type(sp) ~= "number" or sp <= 0 then return true end

    -- Existing StockY margin cap
    local limit = sp * (tonumber(Settings.StockYMargin) or 1.0)
    if price > limit then
        log("STOCKY","SKIP "..shoeName.." "..fmtMoney(price).." > margin cap "..fmtMoney(math.floor(limit)))
        return false
    end

    -- Overpay limiter: skip if price exceeds StockY by more than $X
    if Settings.UseOverpayLimit then
        ...
        if overpay > maxOverpay then
            ...
            BuyStats.Skipped += 1; updateStats()
            return false
        end
    end
...
local function fireAndConfirm(slot, shoeName, price, fromListTab)
    if Settings.DryRun then log("DRY", slot.Name.." | "..shoeName.." | "..fmtMoney(price)) return end
...
local function acquireSlotLock(slot)
    ...
    buyDebounce[slot] = true; return true
end
...
local function walkToPosition(targetPosition)
    ...
    if ok and path.Status == Enum.PathStatus.Success then
        ...
        for _, wp in ipairs(waypoints) do
            if not WalkToBuySnipe and not WalkToBuyOneDollar then return end
...
local function walkToSlot(slot)
...
local function trySneaker(slot)
    ...
    if not stockYOk(shoeName, price) then BuyStats.Skipped+=1; updateStats(); return end
...
local function tryOneDollar(slot)
    ...
    fireAndConfirm(slot, shoeName, price, false)
    releaseSlotLock(slot)
    setStatus("$1 Mode (>$"..OneDollarMaxPrice..")"..(Settings.DryRun and " [DRY RUN]" or " [LIVE]"))
end
...
local function listShoeOnTable(shoeName)
    if not ChangeDisplaySneakerEvent then return false, 0 end
    ...
    local function fillSlots()
        ...
        while runningFn() do
            ...
            if ok then
                ...
                for _ = 1, 20 do task.wait(Settings.AutoSellSlotWait) if getFirstEmptySlot(mt) ~= slot then confirmed=true break end end
                if not confirmed then task.wait(1) break end
            else task.wait(1) break end
        end
    end
...
local function removeShoeFromSlot(slot)
    if RemoveDisplaySneakerEvent then pcall(function() RemoveDisplaySneakerEvent:FireServer(slot) end)
    elseif ChangeDisplaySneakerEvent then pcall(function() ChangeDisplaySneakerEvent:FireServer("", slot.Name, 0) end) end
end
...
local function startWalkBuySnipe()
    ...
    setStatus("Walk-to-Buy Snipe"..(Settings.DryRun and " [DRY RUN]" or " [LIVE]"))
end
...
local function startWalkBuyOneDollar()
    ...
    setStatus("Walk-to-Buy $1 Mode (<="..fmtMoney(OneDollarMaxPrice)..")"..(Settings.DryRun and " [DRY RUN]" or " [LIVE]"))
end
...
local SHAREABLE_SETTINGS_KEYS = {
    ...
    "UseStockYCheck", "StockYMargin", "UnsellableOnly", "DryRun",
...
local function buildConfigSavedList()
    ...
    for _, name in ipairs(names) do
        ...
        local summary = string.format(
            "Max $%s | Delay %.2fs | StockY %s | DryRun %s | BL: %d | SellFallback: $%s | $1Max: $%s",
            commaNum(cfg.MaxPrice or 0),
            tonumber(cfg.Delay) or 0,
            cfg.UseStockYCheck and "ON" or "OFF",
            cfg.DryRun and "ON" or "OFF",
            blCount,
...
local function buildBlacklistTab()
    ...
    if SneakerModuleLoaded and SneakerModule then
        for shoeName, data in pairs(SneakerModule.sneakers) do
            local unsell = table.find(data,"Unsellable") ~= nil
...
BlacklistTab:CreateButton({ Name="Blacklist ALL Unsellable Shoes", Callback=function()
    if not SneakerModuleLoaded then Rayfield:Notify({ Title="Blacklist", Content="Load SneakerModule first.", Duration=4 }) return end
    local count=0
    for n,d in pairs(SneakerModule.sneakers) do if table.find(d,"Unsellable") then Blacklist[n]=true count+=1 end end
    saveBlacklistToDisk()
...
BlacklistTab:CreateButton({ Name="Blacklist ALL Sellable Shoes", Callback=function()
    if not SneakerModuleLoaded then Rayfield:Notify({ Title="Blacklist", Content="Load SneakerModule first.", Duration=4 }) return end
    local count=0
    for n,d in pairs(SneakerModule.sneakers) do if not table.find(d,"Unsellable") then Blacklist[n]=true count+=1 end end
    saveBlacklistToDisk()
...
BlacklistTab:CreateLabel("Requires SneakerModule to be loaded. Uses the Rarity field from the module.")
...
BlacklistTab:CreateButton({
    Name = "Print All Rarity Tags Found in Module",
    Callback = function()
        if not SneakerModuleLoaded or not SneakerModule then
            Rayfield:Notify({ Title="Rarity Debug", Content="Load SneakerModule first!", Duration=4 }) return
        end
        local tagCounts = {}
        for _, data in pairs(SneakerModule.sneakers) do
            local rarity = data["Rarity"]
            if type(rarity) == "string" then tagCounts[rarity] = (tagCounts[rarity] or 0) + 1 end
        end
        local lines = {}
        for tag, count in pairs(tagCounts) do table.insert(lines, tag..": "..count) end
        table.sort(lines)
        print("=== ALL RARITY TAGS IN SNEAKERMODULE ===")
        for _, line in ipairs(lines) do print(line) end
        print("=== END ===")
        Rayfield:Notify({ Title="Rarity Debug", Content=#lines.." unique rarities printed to console (F9).", Duration=5 })
    end
})
...
local function scanAllTables()
    ...
    setStatus("Scanning"..(Settings.DryRun and " [DRY RUN]" or " [LIVE]"))
    local tableCount=0
    for _, tableModel in ipairs(Tables:GetChildren()) do
        if tableModel.Name == "Table" then
            tableCount+=1
...
local function startSnipe()
    ...
    setStatus("Sniping"..(Settings.DryRun and " [DRY RUN]" or " [LIVE]"))
end
...
local function startOneDollar()
    ...
    setStatus("$1 Mode (<="..fmtMoney(OneDollarMaxPrice)..")"..(Settings.DryRun and " [DRY RUN]" or " [LIVE]"))
end
...
--  SCANNER TAB — UI
-- ════════════════════════════════════════
ScannerTab:CreateButton({ Name="[DEBUG] Print Elements Tree", Callback=function()
    local gui
    if type(gethui) == "function" then gui = gethui():FindFirstChild("Rayfield") end
    if not gui then gui = game:GetService("CoreGui"):FindFirstChild("Rayfield") end
    if not gui then
        for _, c in ipairs(game:GetService("CoreGui"):GetChildren()) do
            if c:IsA("ScreenGui") and c:FindFirstChild("Main") then gui = c break end
        end
    end
    if not gui then print("GUI NOT FOUND") return end
    local el = gui:FindFirstChild("Main") and gui.Main:FindFirstChild("Elements")
    if not el then print("ELEMENTS NOT FOUND") return end
    print("=== All ScrollingFrames in Elements ===")
    for i, child in ipairs(el:GetChildren()) do
        if child:IsA("ScrollingFrame") then print(i, child.Name, child.ClassName) end
    end
    print("=== Tab container registry ===")
    local tabs = {
        {ScannerTab,"Scanner"},{ListTab,"List"},{SellTab,"Sell"},
        {BlacklistTab,"Blacklist"},{SettingsTab,"Settings"},
        {ConfigTab,"Configs"},{WebhookTab,"Webhook"},{StatsTab,"Stats"}
    }
    for _, t in ipairs(tabs) do
        local frame = tabContainerRegistry[t[1]]
        print(t[2], "->", frame and (frame.Name.." ("..frame.ClassName..")") or "NIL")
    end
end })
...
ScannerTab:CreateLabel("Respects all filters: Price, StockY, Blacklist, TargetShoe, UnsellableOnly, DryRun.")
...
SettingsTab:CreateToggle({ Name="Dry Run (simulate buys, no actual purchases)", CurrentValue=Settings.DryRun, Callback=function(v) saveSetting("DryRun",v) end })
...
SettingsTab:CreateLabel("Requires: StockY Check ON + StockY cache refreshed. Shoes with no StockY data are still allowed.")
...
SettingsTab:CreateButton({ Name="Reset All Settings to Defaults", Callback=function()
    ...
    Settings.PriceFilter=true; Settings.MaxPrice=100000; Settings.TargetShoe=""
    Settings.Delay=0.05; Settings.UseStockYCheck=false; Settings.StockYMargin=1.5
    Settings.UnsellableOnly=false; Settings.DryRun=false; OneDollarMaxPrice=1
    Settings.SnipeCycleWait=0.5; Settings.OneDollarCycleWait=0.5
    Settings.AutoSellRefillWait=3.0; Settings.AutoSellSlotWait=0.1
    Settings.FlashListHoldTime=0.5; Settings.FlashListDelistTime=0.1
    Settings.BuyConfirmPollWait=0.08
    Settings.UseOverpayLimit=false; Settings.OverpayLimit=0
    saveSettingsToDisk(Settings)
