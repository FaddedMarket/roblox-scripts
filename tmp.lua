...
-- ════════════════════════════════════════
--  STOCK-Y CHECK + OVERPAY LIMITER (FIXED)
-- ════════════════════════════════════════
local function stockYOk(shoeName, price)
    local sp = StockYCache[shoeName]

    -- Overpay limiter runs INDEPENDENTLY — does not require UseStockYCheck to be ON
    if Settings.UseOverpayLimit and sp and type(sp) == "number" and sp > 0 then
    ...
    -- StockY margin cap only runs if StockY check is enabled
...
SettingsTab:CreateLabel("Shoes with no StockY data are still allowed through.")
...
SettingsTab:CreateButton({ Name="Reset All Settings to Defaults", Callback=function()
    ...
    Settings.UnsellableOnly=false; OneDollarMaxPrice=1
